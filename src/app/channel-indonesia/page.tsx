...
const PAGE_URL = `${SITE_URL}/channel-indonesia`;
export const revalidate = 300;

export const metadata: Metadata = {
  title: 'Channel Indonesia',
  description:
    `Daftar lengkap channel TV Indonesia di ${SITE_NAME}. Pilih channel favorit kamu dan tonton live streaming gratis.`,
  alternates: { canonical: PAGE_URL },
  openGraph: {
    siteName: SITE_NAME,
    locale: 'id_ID',
    title: `Channel Indonesia | ${SITE_NAME}`,
    description: `Daftar lengkap channel TV Indonesia di ${SITE_NAME}. Pilih channel favorit kamu dan tonton live streaming gratis.`,
    url: PAGE_URL,
    type: 'website',
  },
  twitter: {
    title: `Channel Indonesia | ${SITE_NAME}`,
    description: `Tonton live streaming channel TV Indonesia gratis di ${SITE_NAME}.`,
  },
};
...
export default function IndonesiaChannelsPage() {
  const indonesiaChannels = readChannels()
    .filter(isIndonesiaChannel)
    .sort((a, b) => a.name.localeCompare(b.name, 'id-ID'));
  const uiSettings = readPublicUiSettings();
  const showGridAd = uiSettings.rightSidebarAd.enabled;
  const gridAdInsertIndex = indonesiaChannels.length >= 16 ? Math.floor(indonesiaChannels.length / 2) : -1;

  const pageJsonLd = buildJsonLd([
    buildWebPage({
      id: `${PAGE_URL}#webpage`,
      url: PAGE_URL,
      name: `Channel Indonesia | ${SITE_NAME}`,
      description:
        `Daftar lengkap channel TV Indonesia di ${SITE_NAME}. Pilih channel favorit kamu dan tonton live streaming gratis.`,
      breadcrumb: [
        { name: 'Beranda', url: SITE_URL },
        { name: 'Channel Indonesia', url: PAGE_URL },
      ],
    }),
    buildWebApplication({
      name: `${SITE_NAME} Channel Indonesia`,
      description: `Tonton live streaming channel TV Indonesia gratis di ${SITE_NAME}.`,
      url: PAGE_URL,
    }),
    buildWebSite(),
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#080a0f] text-gray-900 dark:text-white flex flex-col">
      ...
      <main className="flex-1 max-w-screen-2xl mx-auto w-full px-4 py-8">
        <nav className="mb-6" aria-label="Breadcrumb">
          ...
          >
            ...
            <li
              ...
              itemType="https://schema.org/ListItem"
            >
              <span className="text-gray-400 dark:text-gray-300" itemProp="name">Channel Indonesia</span>
        ...
        <PageTopDesktopAd
          ...
          id="ad-slot-channel-indonesia-top-desktop"
          mobileId="ad-slot-channel-indonesia-top-mobile-inline"
        />
            ...
            <section className="bg-white dark:bg-[#0f1117] border border-black/8 dark:border-white/5 rounded-2xl p-5 sm:p-6">
              <div className="mb-5">
                <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">
                  Semua Channel Indonesia
                </h1>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
                  Total {indonesiaChannels.length} channel Indonesia tersedia.
                </p>
              </div>

              {indonesiaChannels.length === 0 ? (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Belum ada channel Indonesia yang tersedia.
                </p>
              ) : (
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {indonesiaChannels.map((channel, index) => (
                    <div key={channel.id} className="contents">
                      {showGridAd && index === gridAdInsertIndex && (
                        <div className="sm:col-span-2 lg:col-span-3 xl:col-span-4 py-2">
                          <div className="bg-gray-50 dark:bg-white/5 border border-dashed border-black/10 dark:border-white/10 rounded-xl p-4 flex flex-col items-center justify-center min-h-[100px]">
                            ...
                            <div className="w-full flex justify-center">
                              <RightSidebarAdSlot
                                ...
                                id="ad-slot-grid-indonesia-mid"
                                iframeTitle="Grid Ad Indonesia"
          ...
          <aside className="hidden lg:block w-[320px] flex-shrink-0">
            <div className="sticky top-20 flex flex-col gap-5">
              <DesktopSidebarFlexibleAd
                ad={uiSettings.rightSidebarAd}
                id="ad-slot-channel-indonesia-sidebar"
