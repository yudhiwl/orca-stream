...
const MAX_SPORT_CHANNELS = 6;
const ALL_SPORT_CHANNELS_URL = '/channel-sport';
...
interface SportChannelsSectionProps {
  channels: Channel[];
}

export default function SportChannelsSection({ channels }: SportChannelsSectionProps) {
  const sportChannels = channels
    .filter(isSportChannel)
    .sort((a, b) => a.name.localeCompare(b.name, 'id-ID'));
  const featuredChannels = sportChannels.slice(0, MAX_SPORT_CHANNELS);
    ...
    <div className="mt-10 border-t border-black/5 dark:border-white/5 pt-8">
      <div className="mb-2 flex items-center justify-between gap-3">
        ...
        {sportChannels.length > 0 && (
          <Link
            href={ALL_SPORT_CHANNELS_URL}
            className="shrink-0 text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:underline"
          >
            Lihat semua channel sport
          </Link>
        )}
      </div>
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-5">
        Pilihan channel kategori sport yang bisa langsung kamu tonton.
      </p>

      {sportChannels.length === 0 && (
        <p className="text-sm text-gray-500 dark:text-gray-400">Belum ada channel sport yang tersedia.</p>
      )}
